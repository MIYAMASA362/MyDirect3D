...
const int Ground_ReturnNum = MF_GROUND_GRID_WIDTH - 1;
const int Ground_VertexNum = ((MF_GROUND_GRID_WIDTH + 1) * 2)*MF_GROUND_GRID_WIDTH + (2 * Ground_ReturnNum);
...
namespace NMeshField
{
	...
	void Init_Ground()
	{
		...
		for(int i = 0; i< (MF_GROUND_GRID_DEPTH + 1); i++)
		{
			...
			{
				g_pGround_Vertex[nCount + j].Position = {x, 0, z};
				g_pGround_Vertex[nCount + j].Normal = {0,-1, 0};
				g_pGround_Vertex[nCount + j].Coord = { (float)j,(float)i };
				x += fWidth;
				nCount++;
			}
